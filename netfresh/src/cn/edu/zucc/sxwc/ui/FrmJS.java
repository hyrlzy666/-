...
public class FrmJS extends JDialog implements ActionListener{
	...
	public float endprice(String couponid){
		...
		String sql="select jmmoney from coupon where `id`=?";
		java.sql.PreparedStatement pst=conn.prepareStatement(sql);
	...
	public FrmJS(JDialog f, String s, boolean b) {
		...
		for(int i=0;i<address.size();i++) {
			cmbaddress.addItem(address.get(i).getAddid()+" "+address.get(i).getProvince()+address.get(i).getCity()+address.get(i).getAddress()+" "+address.get(i).getContact()+" "+address.get(i).getPhonenum());
		}
