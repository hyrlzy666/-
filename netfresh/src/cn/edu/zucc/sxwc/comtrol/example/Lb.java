...
public class Lb {
	public List<BeanLb> loadLb()throws BaseException{
		...
		try {
			...
			String sql="select lbid,lbname,described from category order by lbid";
	...
	public void createLb(BeanLb lb) throws BaseException{
		...
		try {
			conn=DBUtil.getConnection();
			String sql="select * from category where lbid=?";
			...
			rs.close();
			pst.close();
			sql="insert into category(lbid,lbname,described) values(?,?,?)";
