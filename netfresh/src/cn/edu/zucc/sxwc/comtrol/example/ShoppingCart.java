...
public class ShoppingCart {
	public List<BeanShoppingCart> loadCart()throws BaseException{//购物车
		...
		try {
			...
			String sql="select a.goodsid,a.lbid,a.goodsname,a.price,a.amount,a.sumprice,b.discount,b.syamount "
					+ "from shoppingtrolley a,discount b "
			...
			while(rs.next()){
				...
				if(rs.getInt(5)>=rs.getInt(8)) {
					...
					String sql1="update shoppingtrolley set sumprice=? where userid=? and lbid=? and goodsid=?";
	...
	public BeanShoppingCart addcart(BeanGoods goods,int amount) throws BaseException{
		...
		try {
			...
			String sql1="select * from shoppingtrolley where userid=? and goodsid =? and lbid=? ";
			java.sql.PreparedStatement pst1= conn.prepareStatement(sql1);
			...
			if(rs.next()) {
				...
				String sql="update shoppingtrolley set amount=amount+? where userid=? and goodsid =? and lbid=?";
				java.sql.PreparedStatement pst=conn.prepareStatement(sql);
				pst.setInt(1, num);
				...
				String sql = "insert into shoppingtrolley(goodsid,lbid,goodsname,price,amount,userid) values(?,?,?,?,?,?)";
	...
	public void deletecart(String userid,String goodsid,String lbid)throws BaseException{
		...
		try {
			conn=DBUtil.getConnection();
			String sql="delete from shoppingtrolley where userid=? and goodsid =? and lbid=?";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
	...
	public float totalprice() {
		...
		try {
			conn = DBUtil.getConnection();
			conn.setAutoCommit(false);
			String sql = "select price,amount from shoppingtrolley where userid = ?";
			java.sql.PreparedStatement pst = conn.prepareStatement(sql);
			pst.setString(1, BeanUser.currentLoginUser.getUserid());
			java.sql.ResultSet rs = pst.executeQuery();
			while(rs.next()) 
	...
	public float totalprice1() {
		...
		try {
			conn = DBUtil.getConnection();
			conn.setAutoCommit(false);
			String sql = "select sumprice from shoppingtrolley where userid = ?";
			java.sql.PreparedStatement pst = conn.prepareStatement(sql);
			pst.setString(1, BeanUser.currentLoginUser.getUserid());
			java.sql.ResultSet rs = pst.executeQuery();
			while(rs.next()) 
	...
	public void placeorder(String addid,String couponid) throws BaseException{
		...
		try {
			...
			rs.close();
			pst.close();
			sql="select jmmoney from coupon where id=?";
			pst=conn.prepareStatement(sql);
			...
			sql="select goodsid,lbid,amount,price from shoppingtrolley where userid=? ";
			pst=conn.prepareStatement(sql);
			pst.setString(1, BeanUser.currentLoginUser.getUserid());
			rs=pst.executeQuery();
			while(rs.next()) {
				...
				String sql2="select discountid,discount from discount where goodsid=? and lbid=? and syamount<=?";
			...
			sql="select goodsid,lbid,amount from shoppingtrolley where userid=?";
			pst=conn.prepareStatement(sql);
			pst.setString(1, BeanUser.currentLoginUser.getUserid());
			rs=pst.executeQuery();
			while(rs.next()){
			...
			sql = "delete from shoppingtrolley where userid = ?";
			pst = conn.prepareStatement(sql);
			pst.setString(1, BeanUser.currentLoginUser.getUserid());
			pst.execute();
			pst.close();
			sql="delete from coupon where id=? and userid=?";
			pst = conn.prepareStatement(sql);
			pst.setString(1, couponid);
			pst.setString(2, BeanUser.currentLoginUser.getUserid());
			pst.execute();
